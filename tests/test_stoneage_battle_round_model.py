...
class BattleRoundModelTests(unittest.TestCase):
    ...
    def test_combo_immediate_reflect_ultimate_returns_are_discarded(self):
        p1=actor("p1","player","player",hp=30,attack=500,quick=100)
        p2=actor("p2","player","pet",hp=30,attack=500,quick=90)
        enemy=actor("enemy","enemy","enemy",hp=300,defense=20,quick=10)
        prepared=prepare_battle_round(
            (p1,p2,enemy),
            {
                "p1":BattleCommand(BATTLE_COM_ATTACK,command2=10),
        ...
        combo_entries=tuple(
            replace(
                entry,
                command=BattleCommand(BATTLE_COM_COMBO,command2=10),
                combo_id=1,
            )
            if entry.participant.participant_id in {"p1","p2"}
            else entry
        ...
        result=resolve_ordinary_round(
            ...
            base_damage_react_state_by_participant_id={
                "p1":BaseDamageReactState(),
                "p2":BaseDamageReactState(),
                "enemy":BaseDamageReactState(reflect=2),
            },
            defense_profile="newpower_70pct",
        )
        combo=[event for event in result.events if event.is_combo]
        self.assertEqual(len(combo),2)
        self.assertEqual(
            [event.ultimate_damage_resolution.ultimate_kind for event in combo],
            [2,2],
        )
        self.assertEqual([event.ultimate_kind for event in combo],[0,0])
        self.assertEqual(result.ultimate_exited_participant_ids,())
        self.assertEqual(result.hp_by_participant_id["p1"],0)
        self.assertEqual(result.hp_by_participant_id["p2"],0)
        self.assertEqual(result.hp_by_participant_id["enemy"],300)

    def test_combo_final_reflect_misroutes_damage_sub2_ultimate_flag(self):
        p1=actor("p1","player","player",hp=100,attack=500,quick=100)
        p2=actor("p2","player","pet",hp=30,attack=500,quick=90)
        enemy=actor("enemy","enemy","enemy",hp=30,defense=20,quick=10)
        prepared=prepare_battle_round(
            (p1,p2,enemy),
            {
                "p1":BattleCommand(BATTLE_COM_ATTACK,command2=10),
        ...
        settlement=[
            event for event in result.events
            if event.is_combo and event.combo_settlement
        ][0]
        self.assertEqual(
            settlement.ultimate_damage_resolution.ultimate_kind,
            2,
        )
        self.assertEqual(settlement.ultimate_kind,2)
        self.assertEqual(settlement.resolved_target_slot,10)
        self.assertEqual(settlement.ultimate_flag_target_slot,1)
        self.assertEqual(settlement.ultimate_flag_kind,2)
        self.assertEqual(result.hp_by_participant_id["enemy"],0)
        self.assertEqual(result.ultimate_exited_participant_ids,("p2",))
        self.assertNotIn("enemy",result.ultimate_exited_participant_ids)
    ...
    def test_player_ultimate_exit_is_immediate_and_skips_default_pet_turn(self):
        player=replace(
            actor(
                "player","player","player",
                hp=30,defense=20,quick=50,
            ),
            max_hp=30,
        )
        pet=actor(
            "pet:0","player","pet",
            hp=100,attack=200,quick=40,
        )
        enemy=actor(
            "enemy","enemy","enemy",
        ...
        self.assertEqual(result.hp_by_participant_id["player"],1)
        pet_event=[
            event for event in result.events
            if event.participant_id=="pet:0"
        ][0]
        self.assertEqual(pet_event.result,"skipped_exited")
        self.assertEqual(result.hp_by_participant_id["enemy"],100)

    def test_player_ultimate_exit_clears_common_status_and_petfall_flag(self):
        player=replace(
            actor(
                "player","player","player",
                hp=30,defense=20,quick=50,
        ...
        pet=actor("pet:0","player","pet",hp=1,quick=40)
        enemy=actor(
            "enemy","enemy","enemy",
            hp=100,attack=200,defense=20,quick=100,
        )
        ...
        result=resolve_ordinary_round(
            ...
            base_status_runtime_by_participant_id={
                "player":BaseBattleStatusRuntime(
                    status=BaseBattleStatusState(poison=3),
                    poison_stat_sum=400,
                    work_quick=25,
                    damage_count=2,
                ),
                "pet:0":BaseBattleStatusRuntime(
                    status=BaseBattleStatusState(sleep=2),
                    work_quick=20,
                ),
                "enemy":BaseBattleStatusRuntime(work_quick=100),
        ...
        self.assertEqual(
            result.base_status_runtime_by_participant_id["player"].status,
            BaseBattleStatusState(),
        )
        self.assertEqual(
            result.base_status_runtime_by_participant_id["pet:0"].status,
            BaseBattleStatusState(),
        )
        self.assertFalse(result.ride_pet_runtime.petfall)
        self.assertFalse(result.ride_pet_runtime.mounted)
    ...
    def test_counter_damage_splits_to_active_ride_pet(self):
        rider=actor(
            "rider","player","player",
            hp=200,attack=80,defense=40,quick=100,
        )
        enemy=actor(
            "enemy","enemy","enemy",
            hp=200,attack=80,defense=20,quick=90,
        )
        prepared=prepare_battle_round(
            (rider,enemy),
            {
                "rider":BattleCommand(BATTLE_COM_ATTACK,command2=10),
                "enemy":BattleCommand(BATTLE_COM_ATTACK,command2=0),
            },
            {"rider":0,"enemy":0},
        )
        result=resolve_ordinary_round(
            prepared,
            slots={"rider":0,"enemy":10},
            profiles={"rider":profile(),"enemy":profile()},
            attack_rolls={
                "rider":OrdinaryAttackRolls(
                    dodge_roll_1_10000=10000,
                    critical_roll_1_10000=10000,
                    damage_roll=0,
                ),
        ...
        counter=counters[0]
        self.assertIsNotNone(counter.ride_damage_split)
        self.assertTrue(counter.ride_damage_split.shared)
        self.assertLess(result.hp_by_participant_id["rider"],200)
        self.assertLess(result.ride_pet_runtime.hp,100)
        self.assertTrue(result.ride_pet_runtime.mounted)

    def test_counter_ride_pet_fall_stops_sharing_on_later_counter(self):
        rider=actor(
            "rider","player","player",
            hp=300,attack=80,defense=40,quick=100,
        )
        enemy=actor(
            "enemy","enemy","enemy",
            hp=300,attack=80,defense=20,quick=90,
        )
        prepared=prepare_battle_round(
            (rider,enemy),
            {
        ...
        counters=[
            event for event in result.events
            if event.is_counter and event.result.startswith("counter_")
        ]
        damaging=[event for event in counters if event.damage>0]
        self.assertGreaterEqual(len(damaging),3)
        self.assertTrue(damaging[0].ride_hp_resolution.unmounted)
        self.assertEqual(damaging[0].ride_pet_fell_rider_id,"rider")
        self.assertIsNone(damaging[2].ride_damage_split)
        self.assertFalse(result.ride_pet_runtime.mounted)
        self.assertTrue(result.ride_pet_runtime.petfall)
        self.assertEqual(result.ride_pet_runtime.hp,0)
