...
class T(unittest.TestCase):
    def test_exact_and_1021_event_independence(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            c,m,u,x=analyze(d,s,[100,1021])
            self.assertEqual(c["both_exact"],2);self.assertEqual(c["tile_diff_cells"],0);self.assertEqual(c["parts_diff_cells"],0)
            self.assertEqual(u[0][0],"1021.dat")
            self.assertEqual(x[0][0],100);self.assertEqual(x[0][4:6],(0,0))
            self.assertEqual(x[1][0],1021);self.assertEqual(x[1][4:],(0,0,1))
