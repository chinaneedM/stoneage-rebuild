...
def row(tempno=1,initnum=10):
    ...
    vals[0]=tempno; vals[1]=initnum; vals[2]="5.5"
...
class EnemyBaseProbeTests(unittest.TestCase):
    def test_structural_parse_and_active_file(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(files[0]["profiles"][0][4],2)
            self.assertEqual(files[0]["profiles"][6][1],2)
            self.assertEqual(files[0]["profiles"][8][2],2)
            ...
            self.assertIn("STAT|enemybase.txt|LVUPPOINT|min=5.5|max=5.5|unique=1",buf.getvalue())
            self.assertIn("COLUMN_PROFILE|enemybase.txt|7|integer=2|decimal=0|empty=0|text=0|missing=0",buf.getvalue())
