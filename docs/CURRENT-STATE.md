...
## Recovered 2003 field-map bridge — 2026-09-24

- A Wayback replay of the contemporaneously linked mirror `http://www.wuxitianlong.com/sa/map.exe` at timestamp **2003-06-23 23:44:51 UTC** is now byte-recoverable and hash-locked as SHA-256 `372426e46765a1cf479041bdafc1d3e58fb019117d00d0b43d6a5f796620776e`. Original payload bytes remain transient and are not committed.
- The recovered archive contains the same **1,008 numeric DAT map IDs** as the preserved mixed 2.5 map directory. Of those, **993 are byte-identical** and **15 differ**, with **0 IDs unique to either side**. Both corpora have **995 valid strict three-plane DAT maps** and **13 numeric DAT files that fail that parser**. This materially authenticates most of the preserved 2.5 map directory as carrying bytes already present in a dated 2003 historical map package, while **not** authenticating the rest of the 2.5 bundle or promoting the map package to Taiwan-v1 provenance.
- Plane-level comparison of the 15 changed maps shows bounded changes across tile/parts/event planes rather than a wholesale format change. Examples include map 117 differing by only **one event cell**, while larger world maps such as 200 contain thousands of changed cell values.
- Direct classification of the dated 2003 corpus against the accepted Taiwan-v1 ADRN collision profile yields **773 asset-compatible** and **222 asset-incompatible** maps among the 995 parseable maps; **16 DAT files** in the full extracted scan are invalid for the strict parser. The preserved 2.5 bridge previously yielded 769/226, so four maps cross the compatibility boundary between the dated 2003 bytes and the later preserved bytes.
- The four maps that cross from 2003 `compatible` to preserved-2.5 `incompatible` are **1000, 2000, 3000 and 4000**. All four retain identical tile planes; the later bytes change only the parts plane for 1000/2000/4000, while 3000 changes parts plus event. Their later-only resource IDs cluster around `11208`, `22310/22356/22413/22434` and `24305–24322`. This is consistent with later object/event overlay evolution rather than a replacement of the underlying terrain grid, but the semantic identity of those object IDs remains OPEN.
- **EVIDENCE BOUNDARY:** “asset-compatible” is only a necessary condition. A 2003 map that uses exclusively Taiwan-v1-resolvable tile/parts IDs is **not** evidence that its layout, event plane, or bytes existed in Taiwan v1. The Taiwan-v1 historical map adapter therefore remains unpopulated.
- Derived reports: `research/recovered/STONEAGE-2003-MAP-VS-25-R1.txt` and `research/recovered/STONEAGE-TW10-2003-FIELDMAP-COMPAT-R1.txt`.

## Immediate next actions

1. **Use the recovered 2003-06 map package as a dated descendant bridge to narrow field-map lineage, but keep the Taiwan-v1 historical adapter empty until a pre-/near-v1 provenance anchor is recovered.** The strict DAT plane parser, derived collision-profile loader and composition adapter are already closed. The 4 compatibility-transition maps are now isolated as 1000/2000/3000/4000; the next map task is to use their object/event deltas and the dated 2003 anchor to prioritize **earlier 2002/2001 map-pack/cache mirrors**; do not back-project 2003 bytes into v1.
