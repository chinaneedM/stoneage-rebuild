...
- The full Japanese 1.74a client payload remains unrecovered. The exact next targets are now `sadl.asp` and launch-window `stoneage.to` snapshots rather than speculative installer filenames.
...
3. **Use Taiwan 1.0 as the comparison anchor for future artifact recovery, but do not let broad archaeology block implementation.** JSS 1999 and Korean 1.74 remain high-value provenance targets; for Japanese 1.74a, prioritize the now-recovered official `sadl.asp` / `stoneage.to` launch-window download chain over speculative installer-name searches.
