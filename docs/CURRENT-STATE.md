...
1. **Continue recovered-byte reverse engineering.** REAL/ADRN/RD, SPR/SPRADRN, the main DAT runtime semantics, and the `1021.DAT` anomaly boundary are now resolved as far as this mixed 2.5 bundle permits. The next primary technical target is to classify the **ADRN-unresolved tile/parts ID ranges** and distinguish resource-version gaps from private-server additions/special IDs; after that, move outward into character/pet/item/skill/stat/combat/progression tables as recoverable evidence permits. Keep `1021.DAT` quarantined rather than inventing hidden event types. Treat the single-layer `.MAP` family as SACH/external-tool data unless independent client evidence overturns that classification. In parallel continue the dedicated `〖2.5纯净〗` thread, Korean **1.74**, Japanese **1.74a**, and JSS recovery.
