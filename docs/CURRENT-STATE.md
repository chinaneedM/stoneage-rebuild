...
- **Still OPEN:** field 5 is extracted into a temporary parser buffer but has no recovered persistent record member/downstream semantic use; the exact payload contents carried by the two historical `IP` lines remain unknown without a surviving manifest.
