...
- The dedicated WeLoveSA **`〖2.5纯净〗石器客户端` / `tid=2132`** lead remains publicly indexed and active: the current forum index shows **45 replies / 991 views**, with the latest listed reply at **2026-09-16 01:02** by `mh713`.
- A fresh exact public-index pass over the title, author `rayrix`, `tid=2132`, Discuz archiver/WAP patterns and common file-host terms still exposes **no client filename, attachment ID, share ID, archive size or checksum**. No login, forum-coin purchase or access-control bypass was attempted. The thread remains a live but access-gated lead; `纯净` remains only a source title, not verified cleanliness.
- A new independent modern recovery lead surfaced on CangBaoWan:
  - post `https://www.cangbaowan.vip/thread-9642-1-1.html`;
  - title **`石器时代2.5版，网盘里翻出来的`**;
  - author `saiya141`, posted **2025-09-26 09:11:13**;
  - the unauthenticated/public post exposes exact Baidu share **`https://pan.baidu.com/s/1a2cOmPxo5GjFPFfU5Mj2Ug`**.
- The exact new recovery token is **`1a2cOmPxo5GjFPFfU5Mj2Ug`**. The extraction code is behind the forum's 50-coin topic boundary and is not publicly shown. Exact searches for the share ID, complete URL, title and title+author currently expose no independent public repost containing the extraction code, filename, file size, checksum or file tree.
- **EVIDENCE BOUNDARY:** this CangBaoWan/Baidu object is a modern community/cloud-drive lead, not 2002 operator provenance. No payload bytes or metadata beyond the exact share ID have been recovered, so it does **not** change the pre-June-2003 field-map provenance anchor.
- Operational consequence:
  - stop broad-search repetition on `tid=2132`;
  - monitor/research the exact Baidu share ID, thread `9642`, `saiya141`, or any new filename/hash/mirror token;
  - if bytes become legitimately public, immediately fingerprint and compare them against the recovered MediaFire 2.5 mixed bridge and the 99ds 2026 descendant control before assigning any clean-client grade.
- Canonical sources:
