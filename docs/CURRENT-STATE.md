...
1. **Recover the installed Taiwan v1.0 file set hidden behind the InstallShield cabinet and then inventory/diff maps and other client data.** `sa_3.exe` references `map\\%d.dat`, while the directly visible disc tree has no ordinary map directory; parse `data1.hdr/data1.cab` with an InstallShield-aware extractor before drawing conclusions about missing map assets.
