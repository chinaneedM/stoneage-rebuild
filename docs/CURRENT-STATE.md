...
1. **Continue recovered-byte reverse engineering.** REAL/ADRN/RD, SPR/SPRADRN, and the main `.DAT` runtime-cache semantics are now validated/corroborated. The next primary technical subtargets are: isolate why **`1021.DAT` alone contains 43,952 non-enum event cells**; resolve the remaining small tile/parts control values and ADRN-unresolved graphic-ID ranges; then continue outward into character/pet/item/skill/stat/combat/progression tables as recoverable evidence permits. Keep the single-layer `.MAP` family in the SACH/external-tool track unless independent client evidence overturns that classification. In parallel continue the dedicated `〖2.5纯净〗` thread, Korean **1.74**, Japanese **1.74a**, and JSS recovery.
