...
- **Hananet STAD provides two exact distribution records dated 2001-02-10:** `온라인게임 스톤에이지 정식 버전` (official/full version), record **8119**, size **260 M**; and `온라인게임 스톤에이지 체험 버전` (trial version), record **8120**, size **240 M**. The STAD page explicitly says game-data downloads should be taken from the corresponding GameNet game page. The individual record bodies currently replay as 404, so Hananet filenames/attachment URLs are still unresolved. The 260 M full-version size is close to CNET's 257MB `stoneage.zip`, but byte identity or even identical packaging must not be assumed without a file/hash match.
...
- **Current archive boundary for those exact Hananet/Gagamel targets:** the Hananet flashlink wrapper is archived and frames the exact PDS record, but the inner PDS record variants replay 404; exact Availability/direct-replay checks for `stoneage.hananet.net/down/sa.exe` and `stoneagebeta.zip` produced no recoverable binary snapshot/prefix. These are Wayback-specific negative results, not proof that mirror copies no longer survive elsewhere.
