...
- The **structural grammar of `newest.txt` is now partially closed from first-party code plus bounded helper emulation**:
  ...
  - before key dispatch, colon field 2 is copied into per-record working storage and passed to helper RVA `0x2060`; fields 3 and 4 pass through the same indirect numeric-conversion path and are stored as integers.
- **Still OPEN:** the exact semantics/order of fields 2–5 (filename/generation/version/checksum/size/update-state roles), the role of field 5, and the exact `IP` / `IP:1` / `MESSAGE` control-line semantics. In particular, literal `IP:1` is syntactically awkward under the verified first-colon split, so it must not yet be presented as a proven on-wire record key; it may be legacy/dead logic or depend on an unrecovered transformed representation.
...
- Validation includes launcher **35781291855**, deep structure **35948368684**, semantic **35948545627**, xref **35948787670**, parser/call-argument **35951460845 / 35951653442**, helper semantics **35951664729**, token emulation **35951529320**, checksum-rule verification **35951872034**, launch-parameter **35951951617**, manifest grammar **35954056510**, manifest-line emulation **35954129823**, and updater-path **35781723407** — all successful final runs. Earlier failed/cancelled CI attempts are not evidence.
