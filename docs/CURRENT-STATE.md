...
1. **Build the deterministic Taiwan v1.0 client-data boundary and inventories.** Classify every accepted-client subsystem as disc-resident, runtime-generated/downloaded, executable-embedded, or absent/server-side. Start with maps/battle maps, REAL/ADRN graphics, SPR/SPRADRN animation groups, UI/palettes, BGM/SFX, executables/runtime and local config/save artifacts; then identify which character/pet/item/skill/NPC/master tables are not present client-side and must be reconstructed from protocol evidence, preserved server bridges or later lineage controls.
2. **Turn the inventory into reconstruction-ready datasets.** Preserve stable v1.0 resource IDs/hashes and explicit provenance; map battle scenes, graphics/animation groups and audio first, then correlate protocol-visible character/pet/item/skill fields with the preserved 2.5 bridge without treating later server data as original v1.0 fact.
