...
- Preservation probe run **36096287039 attempt 2** completed successfully for the initial photo-transcription strings and Waei/StoneAge title variants: **0 strict DiscMaster hits / 0 strict Internet Archive items / 0 errors**. This bounds only those tested strings; it does not validate the transcription itself. Checksum-consistent hypotheses must remain separately labelled and tested before this identifier route is closed.
