...
1. **Resolve the Taiwan v1.0 login/server-selection chain beyond the now-confirmed Winsock layer.** Recover the exact business callsites for `inet_addr`, `gethostbyname`, `connect`, `htons`, and `socket`; group them into WGS/game-server connection paths; then trace the host/IP/port values backward to their source. Test the competing possibilities explicitly: embedded constants/table, `waei.bin`, runtime WGS response, config, or launcher handoff.
2. **Build deterministic inventories from the accepted client:** maps/battle maps, sprite/animation groups, characters/pets where recoverable client-side, UI/text/audio assets, executable/runtime files and any client-side item/skill/config tables. Explicitly mark server-side data that the client does not contain.
