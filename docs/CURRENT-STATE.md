...
1. **Reverse engineer the accepted Taiwan 1.0 runtime immediately.** Determine the roles and relationship of `StoneAge.exe` and `sa_3.exe`: PE sections/imports, embedded strings/config names, server/update logic, launcher/install behavior and byte/code similarity. Promote only independently observed early-client traits.
2. **Parse the Taiwan 1.0 resource generation and compare it against the recovered 2.5 bridge.** Start with `real_1.bin / adrn_1.bin / spr_1.bin / spradrn_1.bin`, `battle_1.bin`, `battle*.sab`, palette/address tables and sound indexes. Establish container/index schemas before interpreting game semantics.
