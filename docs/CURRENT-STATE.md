...
1. **Continue moving the field-map provenance anchor earlier than June 2003 without back-projecting descendant bytes.** The 2002-11-08 4.0 package remains the best concrete earlier target but its exact filename, preservation-carrier and Sina-CGI archive surfaces are now substantially bounded with no byte recovery. Use the June/December mtime+CRC strata and three-state branch comparison only to prioritize which early map IDs/caches to seek in 2001–2002 installed-tree backups, full-map packages, mirrors or server-captured caches; do not treat old mtimes as release dates.
