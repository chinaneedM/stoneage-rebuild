...
- **GamePia:** 12 carriers across issues **No.58–No.64**, crossing the Korean 2000 trial/formal launch window, were fully enumerated with **0 scan errors, 0 truncation and 0 StoneAge-path hits**.
...
- **New GameTime lead:** CDX preserves a **2001-07-01** `category=online` search page whose CP949 query decodes exactly to `스톤에이지`. This is now a concrete archived StoneAge result surface and the highest-value unresolved GameTime branch for recovering the underlying record/detail/download identity and testing its relationship to the independently known `GW_IDX=9`.
...
2. **Resolve the archived GameTime StoneAge result chain before broadening media searches.** The 2001-07-01 archived GameTime search page explicitly targets `스톤에이지`; extract its result-row identifiers, detail/download endpoints and relation to `GW_IDX=9`, then probe any newly resolved exact URL for surviving captures/bytes. The GameTime guide union-catalog route is closed at the bibliographic layer and should reopen only for holder-specific supplementary-disc status, disc imagery, a distinct non-book record or a public preservation copy.
