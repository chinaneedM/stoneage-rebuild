...
1. **Exploit the Hananet 2001-02-10 full/trial records and recover the corresponding GameNet download targets.** Hananet is now pinned to record **8119 / 260 M** (official/full) and **8120 / 240 M** (trial), while CNET is pinned to `Software_Id=200009263856`, **257MB**, `/pc/games/online/stoneage.zip`. Target GameNet/StoneAge snapshots around 2001-02-10 and the later STAD snapshot, recover any filename/attachment/redirect, then compare it against CNET before broad searching. Reject unrelated arcade/MAME `stoneage.zip` hits.
