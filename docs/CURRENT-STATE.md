...
1. **Recover the exact CNET payload bytes or a byte-identical mirror.** The historical artifact is now pinned to `Software_Id=200009263856`, size label **257MB**, payload path `/pc/games/online/stoneage.zip`. Query exact archive indexes and old mirrors using the full path/size/provenance tuple; reject unrelated arcade/MAME `stoneage.zip` hits. In parallel, continue Inium/Hananet child-path recovery.
