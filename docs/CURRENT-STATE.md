...
- **GameTime StoneAge data-center records are now partially resolved.** The 2001-07-01 `스톤에이지` search page identifies `GW_IDX=76` as `스톤에이지 체험판 클라이언트`, filename **`stone_demo.exe`**, registered **2001-02-12 19:45:00**, list size **234MB**; its description says the formal version is roughly 260MB and the trial version roughly 240MB, with five-day trial access and separate trial characters/pets. The same page identifies `GW_IDX=34` as a **manual update**, filename **`StoneAge.zip`**, registered **2000-11-06 11:21:00**, list size **0.4MB**; an independent 2001-04-17 GameTime page labels it **0.42MB**. Its instructions explicitly mention deleting `sa_*.exe`, `server_*.ini` and `stoneage.exe` before copying the update. Therefore this GameTime `StoneAge.zip` is not the CNET 257MB full-client ZIP despite the same filename. An 11-anchor/52-record online-list sweep found no `GW_IDX=9`, so that remaining formal-mirror identity now moves to the older GameTime webzine/download system rather than further pagination.
- Canonical corpus record: `research/clients/STONEAGE-KOREA-2000-2001-PUBLIC-MEDIA-CORPUS-R1.md`.

## Immediate next actions

1. **Recover bytes or surviving mirrors from the now-exact Korean distribution tokens.** Highest-value keys are CNET `/pc/games/online/stoneage.zip` (257MB), Hananet PDS **`app_id=20001031524596220&type=C03`**, Hananet formal-version direct path **`stoneage.hananet.net/down/sa.exe`**, Hananet trial **`sa_demo.exe`**, Gagamel `stoneagebeta.zip`, GameTime trial **`stone_demo.exe` / GW_IDX=76**, GameTime unresolved formal-mirror **`GW_IDX=9`**, and STAD records **8119/260 M** + **8120/240 M**. Treat GameTime `GW_IDX=34 / StoneAge.zip` as a 0.4/0.42MB manual update, not a full-client candidate. Compare candidate clients only by bytes/hashes and internal trees; reject unrelated arcade/MAME `stoneage.zip` hits.
2. **Resolve GameTime `GW_IDX=9` through the older webzine/download system.** The newer data center has now yielded GW_IDX=76 trial and GW_IDX=34 manual-update identities but no GW_IDX=9 across 52 parsed online records. Probe the preserved 2000-11/2001-01 `webzine/online/down/bbs.asp` pages and `download.asp?name=스톤에이지` transition path, then test any recovered exact attachment URL for surviving captures/bytes. The GameTime guide union-catalog route remains closed at the bibliographic layer unless new supplementary-disc evidence appears.
