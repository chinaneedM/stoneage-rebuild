...
1. **Continue moving the field-map provenance anchor earlier than June 2003 without back-projecting descendant bytes.** The direct Waei 2.5 web/binary surface and the 2002-11-08 4.0 package surface are now substantially bounded without byte recovery. Highest information gain now comes from **named Jan/Feb-2002 cover-disc carriers, contemporaneous installed-tree backups, mirrors/FTP copies, or server-captured caches**, using exact publication/issue identity rather than broad StoneAge keyword searches. Treat the 2001-12-04 `tyro/upgrade.asp` capture only as a StoneAge 2.0 level-up-guide/topology source; do not reinterpret it as a software-upgrade page. Use June/December mtime+CRC strata only for candidate prioritization, never as release dates.
