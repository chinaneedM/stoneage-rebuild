...
1. **Continue recovered-byte reverse engineering.** REAL/ADRN/RD and SPR/SPRADRN are now validated on the recovered 2.5 corpus. The next primary target is the descendant-source-corroborated client `.DAT` map cache: trace and validate the exact semantics of its **tile / parts / event** layers through rendering, collision, movement and network-update code. Treat the single-layer `.MAP` family as SACH/external-tool data unless independent client evidence overturns that classification. In parallel continue the dedicated `〖2.5纯净〗` thread, Korean **1.74**, Japanese **1.74a**, and JSS recovery.
