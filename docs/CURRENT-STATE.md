...
- Verified all **1,030** recovered `.MAP` files use `uint32 width + uint32 height + one uint16 per cell`.
- Separated the recovered map families. The directory contains **1,030 MAP + 1,011 DAT** files; descendant source establishes DAT as a three-layer `tile / parts / event` cache/runtime structure. Byte-level comparison found **995 valid same-stem pairs and zero cases where MAP equals DAT tile, parts or event**, so MAP is a distinct single-layer dataset whose cell semantics remain OPEN.
- Identified the next high-value recovered-byte target: `spradrn_5.bin` is exactly **10,164 bytes = 847 × 12-byte SPRADRN records**, matching the Win32-aligned descendant SPR index structure. Full offset/animation/frame validation is next.
...
- Classified the recovered 2.5 package as **RECOVERED-C for runtime / B-grade for resource archaeology**. The core `stoneage2.5` tree contains 2,302 files / ~863 MB, including `real_15.bin` (~764 MB), `adrn_15.bin` (~18.8 MB), SPR/sound containers and 2,041 client MAP files.
...
1. **Continue recovered-byte reverse engineering.** REAL/ADRN/RD and the basic MAP/DAT structures are now validated on the recovered 2.5 corpus. Next validate all **847 SPRADRN records** against `spr_4.bin`, reconstruct animation/frame indexing, and independently determine the single-layer `.MAP` cell semantics. In parallel continue the dedicated `〖2.5纯净〗` thread, Korean **1.74**, Japanese **1.74a**, and JSS recovery.
