...
- JSS product-identifier archaeology now has two directly grounded Windows online-RPG JAN anchors. Modern Japanese retailer data records **LIFESTORM** as model **`JV02005`**, JAN **`4909476301016`**; a surviving **LIFESTORM II** package photographed in Yahoo! Auctions directly exposes back-box JAN **`4909476302013`**. The LIFESTORM II model/type code remains unresolved. Other JSS-associated Windows products use different `4909476...` subranges (`古都の旅 京都` = `4909476502031`; `四柱推命入門 しゃべる桃源郷` = `4909476603011`), while JSS Game Boy/N64 titles occupy `49094768...`. Therefore console-sequence extrapolation remains rejected, and the next valid PC-sequence number `4909476303010` is retained only as a StoneAge **search candidate**, not an assigned identifier.
...
1. **Locate and verify the earliest recoverable JSS retail install/client medium or provenance-preserving image.**
   ...
   - Product-identifier progress: JSS Windows `LIFESTORM` is cataloged as `JV02005` / JAN `4909476301016`; a newly recovered public Yahoo! Auctions back-box photograph directly fixes **LIFESTORM II JAN `4909476302013`**. This promotes a former numeric search candidate to package-evidenced fact. The LIFESTORM II model/type code is still unread/unresolved, and **StoneAge still has no directly verified JAN/model**. `4909476303010` is therefore only a prioritized exact-string search key.
...
2. Complete the **LIFESTORM II** identifier record by recovering its still-missing model/type code from freely accessible box/disc scans, retailer/distributor databases, auction archives or period catalogs. Its JAN is now directly verified as **`4909476302013`**. Preserve live listings only as public evidence; never pursue acquisition.
