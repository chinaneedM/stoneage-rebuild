...
- **Hananet StoneAge child-site identity is now recovered:** a 2001-01-18 GamePlus page links `[게임Plus]스톤에이지 게임Plus 신규 오픈` to `/gamenet/sitemap/map/mapstoneage.html`, which resolves through `frstoneage.html` to the dedicated service host **`http://stoneage.hananet.net/main.htm`**. The dedicated homepage is archived and exposes menu pages `1.htm`, `2.htm`, `2_2.htm`, `2_3.htm`, `2_4.htm`, `2_5.htm`; `1.htm` is already recovered, while Availability confirms archived snapshots for `2.htm`, `2_2.htm`, `2_3.htm`, and `2_4.htm` at later exact timestamps. Menu-page extraction is the active Hananet sub-track.
...
1. **Recover the exact CNET payload bytes or a byte-identical mirror while exhausting the newly recovered Hananet StoneAge menu chain.** CNET is pinned to `Software_Id=200009263856`, **257MB**, `/pc/games/online/stoneage.zip`; Hananet is pinned to `stoneage.hananet.net` with archived `1/2/2_2/2_3/2_4` menu pages. Extract any Hananet download/install target and compare it against the CNET filename/size before broad searching. Reject unrelated arcade/MAME `stoneage.zip` hits.
