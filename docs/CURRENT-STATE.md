...
1. **Exploit the first recovered bytes without mistaking them for a clean runtime.** The preserved 2.5 bundle is now recovered and classified: runtime = mixed/modified, resource corpus = high-value bridge. First validate REAL/ADRN and representative MAP files against actual recovered bytes; in parallel continue the dedicated `〖2.5纯净〗` thread, Korean **1.74**, Japanese **1.74a**, and JSS recovery.
