...
- The deep import pass corrects an earlier coarse observation: `sa_3.exe` has **no statically imported Winsock DLL/functions** in the verified import table. Network/login semantics are still evidenced by runtime strings and code xrefs, but the socket implementation path is OPEN until dynamic loading, statically linked networking code or another mechanism is demonstrated.
...
1. **Diff Taiwan v1.0 directly against the recovered 2.5 bridge at byte/record level.** Quantify exact inherited ADRN/REAL records and SPR/SPRADRN records, locate the first divergence, and separate append-only growth from modified historical records.
2. **Resolve the remaining Taiwan v1.0 runtime network path narrowly.** `sa_3.exe` has login strings/code xrefs but no static Winsock import in the verified import table; test dynamic `LoadLibrary/GetProcAddress`, embedded networking implementation and related call sites before asserting socket behavior.
