...
1. **Finish the exact Taiwan v1.0 map/login semantic join from the recovered xref roots.** Expand the protocol-name set around `ClientLogin / CreateNewChar / CharDelete / CharLogin` and classify the shared helpers `0x1b060 / 0x1b0f0 / 0x1b3f0 / 0x1b480` by cross-message call signatures. In parallel, distinguish the six `map\\%d.dat` callsites into create/read/write/check paths and join the map write path to the v1.0 receive/dispatch logic. Keep the later source implementation as a lineage control, not as proof of an unobserved v1.0 call.
