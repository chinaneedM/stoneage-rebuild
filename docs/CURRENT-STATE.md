...
- **Still OPEN:** field 5 is definitely extracted into a temporary parser buffer, but no persistent record member or downstream semantic use has yet been recovered; the exact `IP` / `IP:1` / `MESSAGE` control-line semantics also remain open. In particular, literal `IP:1` is syntactically awkward under the verified first-colon split, so it must not yet be presented as a proven on-wire record key; it may be legacy/dead logic or depend on an unrecovered transformed representation.
