...
1. **Resolve the Taiwan v1.0 runtime map-cache and login/server-selection chains.** The InstallShield cabinet has been ruled out as the source of ordinary map assets. Trace pointer indirection and bounded internal call graphs around `map\\%d.dat`, `waei.bin`, `ClientLogin` and `CharLogin`, then join those roots to `CreateFileA/ReadFile/WriteFile` and Winsock calls before classifying maps as cache/download/generated data or asserting how server host/port selection works.
