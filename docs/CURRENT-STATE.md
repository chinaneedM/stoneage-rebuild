...
- The exact v1.0 send-builder graphs still do not directly reach WSOCK32. The later `lssproto_Send` implementation uses an indirect `lssproto.write_func` callback, providing a concrete explanation to test. **Next binary target: inspect v1.0 `0x1b3f0` for its indirect call and trace the backing function pointer initialization to the actual socket/write wrapper.**
...
1. **Resolve the indirect Taiwan v1.0 network handoff from the now-identified protocol generator.** Inspect `0x1b3f0 = lssproto_Send` for its indirect call target/global function pointer, trace that pointer's initialization, and join it to the actual socket/write wrapper. Then resolve the six per-protocol mov-side dispatch branches. In parallel, distinguish the six `map\\%d.dat` callsites into create/read/write/check paths and join the map write path to the v1.0 receive/dispatch logic.
