...
1. **Build the Taiwan v1.0 battle-scene reconstruction dataset.** Preserve the 218 SAB layout identities, the 15 battle-indexed palettes, the separate `Palet_0.sap`, address-table offsets/sizes and file/container hashes so each historical battle-scene resource has an explicit reconstruction key.
2. **Export reconstruction-ready REAL/ADRN and SPR/SPRADRN metadata keyed by stable v1.0 IDs, then build the audio crosswalk.** Preserve geometry/offset/compression metadata for 125,996 graphics records and animation/frame relationships for 464 sprite groups without committing proprietary payloads; for audio, keep the 114 indexed container records, 116 loose SFX WAVs and 11 BGM WAVs as distinct provenance surfaces.
