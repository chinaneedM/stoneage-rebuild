...
- Mainland listing `22636573895893` exposes a photographed Simplified-Chinese WAEI/WGS StoneAge retail box whose back panel visibly gives **ISBN `7-900032-57-0` / barcode `9787900032570`**. This is a new exact physical-package recovery token and is separate from the Wanfang 2.5 disc ISBN/barcode already tracked.
...
- Exact preservation probe run **36096287039 attempt 2** completed successfully for the new Mainland ISBN/barcode and Waei/StoneAge title variants: **0 strict DiscMaster hits / 0 strict Internet Archive items / 0 errors**. The tested DiscMaster/IA exact-identifier surface is therefore bounded and should reopen only from a new corpus, catalogue record, file/disc token, checksum, volume label or public media dump.
