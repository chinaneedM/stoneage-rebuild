...
- **GameTime's near-period StoneAge download identity is also recovered but its payload is not archived.** The archived GameTime StoneAge article `IDX=11` (snapshot 2000-12-08) links a dedicated relative endpoint `../download.asp?name=스톤에이지`, i.e. `/webzine/online/download.asp?name=스톤에이지`. UTF-8 and CP949/EUC-KR encoded direct replays return 404, and 12 zero-error Availability queries across 2000-11 through 2001-08 find no saved snapshot. The later Inium mirror URL `/data/download.asp?GW_IDX=9&GW_Name=Online` likewise replays 404 at known Inium reference timestamps. Treat GameTime as a confirmed historical mirror identity whose download object remains unrecovered.
...
- Internet Archive exact-token reverse search checked **216 item records with 0 request errors** for the known Korean payload identifiers. It produced **0 exact target filename matches**; all nine 180–320 MiB candidates were size-only false positives.
...
- **GameTime StoneAge record lineage is now materially resolved.** The 2001-07-01 `스톤에이지` search page identifies `GW_IDX=76` as `스톤에이지 체험판 클라이언트`, filename **`stone_demo.exe`**, registered **2001-02-12 19:45:00**, list size **234MB**; its description says the formal version is roughly 260MB and the trial version roughly 240MB, with five-day trial access and separate trial characters/pets. `GW_IDX=34` is a **manual update**, filename **`StoneAge.zip`**, registered **2000-11-06 11:21:00**, list size **0.4MB**; an independent 2001-04-17 GameTime page labels it **0.42MB** and tells users to delete `sa_*.exe`, `server_*.ini` and `stoneage.exe` before copying it. The older GameTime webzine archive now adds a **2000-10-11 `스톤 에이지 베타 버젼용 클라이언트`** row with count **7,898**, machine-bound directly to `content.asp?name=&num=9&ref=37&page=4`. The same legacy system machine-binds the StoneAge manual update to `num=34`, while the migrated data center preserves that same object as `GW_IDX=34`; this is strong record-key continuity evidence that Inium's later official mirror link `GW_IDX=9` refers to the earlier GameTime StoneAge Beta client record. The `num=9` detail page and direct StoneAge legacy download query are not preserved (404), so filename/size/version and whether the payload was replaced by the time Inium classified the mirror among formal-version programs remain unresolved. This must not be upgraded to byte identity without payload evidence.
- Canonical corpus record: `research/clients/STONEAGE-KOREA-2000-2001-PUBLIC-MEDIA-CORPUS-R1.md`.

## Immediate next actions

1. **Recover bytes or surviving mirrors from the now-exact Korean distribution tokens.** Highest-value keys are CNET `/pc/games/online/stoneage.zip` (257MB), Hananet PDS **`app_id=20001031524596220&type=C03`**, Hananet formal-version direct path **`stoneage.hananet.net/down/sa.exe`**, Hananet trial **`sa_demo.exe`**, Gagamel `stoneagebeta.zip`, GameTime trial **`stone_demo.exe` / GW_IDX=76**, and GameTime legacy/migrated **`num=9` / `GW_IDX=9`** whose historical title is now resolved to the **2000-10-11 StoneAge Beta-version client** but whose payload filename/size remains unknown. Treat GameTime `GW_IDX=34 / StoneAge.zip` as a 0.4/0.42MB manual update, not a full-client candidate. Compare candidate clients only by bytes/hashes and internal trees; reject unrelated arcade/MAME `stoneage.zip` hits.
2. **Recover the payload behind GameTime record 9.** The older webzine list now resolves StoneAge Beta to legacy `num=9`, and `num=34 → GW_IDX=34` proves record-key continuity across the old/new GameTime systems; Inium later links `GW_IDX=9`. The remaining GameTime gap is therefore the `num=9/GW_IDX=9` attachment filename, size, version and bytes, not the record's historical StoneAge identity. Search exact old/new record URLs, mirrors and reposts; do not assume the 2001 payload remained byte-identical to the 2000 Beta listing. The GameTime guide union-catalog route remains closed at the bibliographic layer unless new supplementary-disc evidence appears.
