...
def load_spreadrn(path: Path):
    ...
    for i in range(count):
        spr_no, offset, anim_size = struct.unpack_from("<IIH", data, i*SPRADRN_RECORD_SIZE)
        rows.append({
            "index":i,
            "spr_no":spr_no,
            "offset":offset,
...
def analyze(spreadrn_path: Path, spr_path: Path, sample_limit: int = 20):
        ...
        for rec in rows:
            ...
            if off > spr_size:
                ...
                if len(failures)<sample_limit:
                    failures.append((idx,rec["spr_no"],off,anim_count,"offset_out_of_bounds"))
                continue
            ...
            try:
                for _ in range(anim_count):
                    ...
                    direction, anim_no, duration, frame_count=struct.unpack("<HHII",hdr)
                    ...
                    for _frame in range(frame_count):
                        ...
                        local_frames += 1
                        frame_total += 1
                        frame_bmp_min=bmp_no if frame_bmp_min is None else min(frame_bmp_min,bmp_no)
                        frame_bmp_max=bmp_no if frame_bmp_max is None else max(frame_bmp_max,bmp_no)
                        if sound_no < 10000:
            ...
            if ok:
                ...
                if len(samples)<sample_limit:
                    samples.append((
                        idx,rec["spr_no"],off,anim_count,local_anims,local_frames,cursor,nxt,relation
                    ))
    ...
    return {
        ...
        "duplicate_sprnos":duplicate_sprnos,
...
def emit(r):
    print("StoneAge recovered SPR/SPRADRN probe — R1")
    print("No proprietary animation payload bytes are stored in this report.")
    for key in [
        "spreadrn_bytes","spr_bytes","record_size","record_count","remainder",
        "sprno_min","sprno_max","offset_min","offset_max","duplicate_sprnos",
        "duplicate_offsets","monotonic_offset_transitions","animation_total",
        "frame_total","frame_bmp_min","frame_bmp_max"
    ]:
    ...
    print("SPR_SAMPLE|index|spr_no|offset|anim_size|parsed_anims|parsed_frames|end|next_offset|relation")
...
def main():
    ...
    args=ap.parse_args()
    result=analyze(args.spradrn,args.spr)
    emit(result)
