...
def analyze(dat_dir,server_root):
    ...
    for p in sorted((x for x in dat_dir.iterdir() if x.is_file() and x.suffix.lower()==".dat"),key=lambda x:x.name.lower()):
        ...
        if not entries: c["no_server_id"]+=1; continue
        ...
        if best is None: c["dimension_mismatch"]+=1; continue
        ...
        if mid==1021: detail1021.append((w,h,sp,td,pd,u))
    c["server_files"]=total;c["server_ls2map"]=ls2;c["server_unique_ids"]=len(server)
    return c,mismatch,unknown,detail1021

def main():
    ap=argparse.ArgumentParser();ap.add_argument("--dat-dir",type=Path,required=True);ap.add_argument("--server-map-root",type=Path,required=True);a=ap.parse_args()
    c,m,u,d=analyze(a.dat_dir,a.server_map_root)
    ...
    for row in d:print("MAP_1021|"+"|".join(map(str,row)))
