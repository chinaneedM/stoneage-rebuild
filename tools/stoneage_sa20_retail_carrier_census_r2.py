#!/usr/bin/env python3
"""Refined preservation census for Mainland StoneAge 2.0 client-disc carriers.

R1's broad IA veteran-short query exceeded the 8 MiB diagnostic body cap.
R2 replaces broad unfielded searches with year/title/description constraints,
adds the visible period label 家族开拓史, and keeps exact Sina filename probes.
Metadata/index only; no historical payload is downloaded.
"""
...
IA_QUERIES=(
 ("newbie-title-2001",'title:("石器时代2.0新手报到包" OR "石器时代2.0 新手报到包") AND year:2001'),
 ("veteran-title-2001",'title:("石器时代2.0老手削暴包" OR "石器时代2.0 老手削暴包") AND year:2001'),
 ("newbie-desc-2001",'description:"新手报到包" AND year:2001'),
 ("veteran-desc-2001",'description:"老手削暴包" AND year:2001'),
 ("family-history-title",'title:"家族开拓史" AND year:2001'),
 ("client-disc-desc",'description:("石器时代2.0" AND "客户端光盘") AND year:2001'),
...
def main():
    print("StoneAge 2.0 retail client-disc preservation census — R2")
    print("SCOPE|IA refined year/title/description metadata + DiscMaster exact label/filename index|no-payload")
    print("TARGET|newbie=石器时代2.0新手报到包|veteran=石器时代2.0老手削暴包|label=家族开拓史|sina=stoneage2.0setup.exe")
        ...
        try:
            ...
            for d in rr:
                ...
                print(
                  ...
                  f"mediatype={clean(d.get('mediatype'))}|description={clean(d.get('description'),900)}"
                )
    ...
    if ia_strict or dm_strict:
        ...
        print("RESOLUTION|NO_STRICT_RETAIL_CARRIER_HIT|refined IA/DiscMaster retail/client-label surface exposes no preserved client-disc candidate")
    print("EVIDENCE_BOUNDARY|index metadata is discovery evidence only; package/label matches do not establish optical pressing or client-byte identity.")
