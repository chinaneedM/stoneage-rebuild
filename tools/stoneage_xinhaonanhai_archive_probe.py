#!/usr/bin/env python3
"""Probe the original contributor domain for the 2001 StoneAge full-map package.

The surviving Sina record attributes the package to xinhaonanhai and links
www.xinhaonanhai.com. This probe enumerates archived URLs and selected home-page
captures only; it does not fetch large binary payloads.
"""
...
def cdx_url(root):
    p=[
      ...
      ("from","2001"),("to","2003"),("filter","statuscode:200"),
...
def relevant_url(u):
    ...
    return (
      TARGET.lower() in s or
      any(x in s for x in ("stoneage","shiqi","shi_qi","1127","2.0map","map_1127","/sa/","/stone/")) or
...
def main():
    print("StoneAge xinhaonanhai archive probe — R1")
    print("SCOPE|contributor-domain-CDX+selected-home-replays|metadata/html-only|no-large-payload")
    print(f"TARGET|filename={TARGET}|source_role=2001-11-27 Sina contributor")
    ...
    for root in DOMAINS:
        ...
        for date in ("20011127","20011207","20011221","20020101","20021108","20030101"):
            try:
